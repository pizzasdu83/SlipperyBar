...
static void HBTApplyOverlay(UIView *pillView) {
    ...
    if (!hbtEnabled) {
        if (overlay && !overlay.hidden) {
            ...
            fadeOut.fromValue = @(overlay.opacity);
            fadeOut.toValue = @0.0;
            fadeOut.duration = 0.2;
            overlay.opacity = 0.0f;
            [overlay addAnimation:fadeOut forKey:@"hbtFadeOut"];
    ...
    if (hbtIsGradient) {
        newColors = @[(id)hbtColor1.CGColor, (id)hbtColor2.CGColor];
        HBTGradientPointsForAngle(hbtAngleDegrees, &newStart, &newEnd);
    } else {
        newColors = @[(id)hbtColor1.CGColor, (id)hbtColor1.CGColor];
        newStart = CGPointMake(0, 0.5f);
...
- (void)sendEvent:(UIEvent *)event {
    ...
    for (UITouch *touch in touches) {
        ...
        BOOL nearHomeBar = (location.y - screenHeight * 0.5) <= 40.0;
